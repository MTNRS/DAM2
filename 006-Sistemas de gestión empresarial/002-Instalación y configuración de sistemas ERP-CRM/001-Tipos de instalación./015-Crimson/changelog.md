...
* Creado el componente `nucleo/componentes/crud.html`.
* Separadas las responsabilidades entre navegación y operaciones CRUD.
* `menu.html` queda limitado a obtener las tablas disponibles y seleccionar la entidad sobre la que trabajar.
* El componente CRUD gestiona de forma genérica las operaciones de selección, inserción, actualización y eliminación.
* Eliminada la dependencia del antiguo componente específico `clientes.html`.
* El área `<main>` queda reservada exclusivamente para mostrar la tabla o el formulario correspondiente.
* El componente CRUD se carga antes que el menú para garantizar que sus funciones estén disponibles durante la navegación.

### Cabecera y contexto de operación

* La cabecera muestra dinámicamente la tabla activa y la operación actual.
* Formato de contexto: `Tabla · operación`.
* Añadidas acciones de `Seleccionar` e `Insertar` en la cabecera.
* Las operaciones de actualización y eliminación se ofrecen desde cada registro de la tabla.

### API CRUD

* Ampliado `api/api.php` con operaciones genéricas sobre cualquier tabla SQLite.
* Añadido endpoint para obtener un registro mediante su `Identificador`.
* Añadido endpoint de inserción.
* Añadido endpoint de actualización.
* Añadido endpoint de eliminación.
* Las operaciones utilizan la estructura real de la tabla para determinar los campos válidos.
* El campo `Identificador` continúa gestionándose automáticamente como clave primaria.
* Añadida comprobación de respuestas y presentación de errores de API en la interfaz.

### Formularios dinámicos

* Los formularios de inserción y actualización se generan automáticamente a partir de la estructura SQLite.
* Eliminada la necesidad de crear formularios específicos para cada entidad.
* Los formularios se representan mediante una tabla de dos columnas:
  * primera columna: nombre del campo;
  * segunda columna: control de entrada.
* La disposición mantiene la alineación visual con las tablas de selección.
* La primera columna se ajusta automáticamente a la anchura máxima necesaria para los nombres de los campos.
* La segunda columna ocupa el espacio restante disponible.
* Añadida la variable CSS global `--gap:10px`.
* Los grupos de campos mantienen una separación vertical de `10px` mediante `var(--gap)`.

### Relaciones y claves foráneas

* La API obtiene las relaciones mediante `PRAGMA foreign_key_list`.
* La estructura devuelta por la API incorpora metadatos de clave foránea.
* Los campos normales continúan representándose mediante `<input>`.
* Los campos FK se representan mediante `<input list>` y `<datalist>`.
* Las opciones de una FK se cargan dinámicamente desde la tabla relacionada.
* El usuario trabaja con valores descriptivos del registro relacionado en lugar de introducir manualmente el identificador.
* Al guardar, el componente transforma el valor descriptivo seleccionado en el `Identificador` correspondiente.
* Al actualizar, el identificador almacenado se transforma nuevamente en su representación descriptiva.
* La gestión de relaciones es genérica y no contiene lógica específica para `Clientes`, `Productos` o `Pedidos`.

### Arquitectura actual

El flujo funcional de Crimson queda organizado de la siguiente forma:

`modelodedatos.md → SQLite → api.php → crud.html → interfaz`

Mientras que la navegación mantiene un flujo independiente:

`SQLite → api.php?bloque=menu → menu.html → crud(tabla, operación)`

Esto permite añadir nuevas tablas y relaciones al modelo de datos y reutilizar automáticamente el mismo componente CRUD.
